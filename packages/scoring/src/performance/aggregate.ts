...
export function computePerformanceDimension(
  ...
  const dungeonSummaries: PerformanceDungeonSummary[] = validDungeons.map((d) => {
    ...
    return {
      ...
      throughputBracket: d.throughputBracket ?? null,
      ratingPoints: d.ratingPoints ?? null,
      scoreRank: d.scoreRank ?? null,
      regionRank: d.regionRank ?? null,
      serverRank: d.serverRank ?? null,
      scoreRankPercent: d.scoreRankPercent ?? null,
  ...
  const summary: PerformanceSummaryDTO = {
    currentSeason: {
      ...
      confidence,
      dungeonCount: validDungeons.length,
      availableDungeonCount: validDungeons.length,
      expectedDungeonCount: input.expectedDungeonCount,
      latestObservedAt,
