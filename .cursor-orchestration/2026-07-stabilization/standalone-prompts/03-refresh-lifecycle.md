# Standalone Cursor prompt — Agent 03

This prompt is self-contained. You are operating inside the dedicated Git worktree
for branch `agent/03-refresh-lifecycle`.

You are Agent 03: Refresh Lifecycle Stabilization for the M+ Trust Factor repository.
...
Implement a strict, centralized, testable 7-day Trust Score refresh policy.
Eliminate refresh storms caused by profile/account navigation.
...
- response cache
- account/profile routes and frontend polling
- `apps/worker/src/orchestration/enqueue.ts`
- `apps/worker/src/dedupe.ts`
- job repository
- `packages/database/prisma/schema.prisma`
- refresh contract logic
- owned-character discovery enqueue behaviour

Current code can enqueue from:
- missing snapshot;
- expired `lastPublicRefreshAt`;
- provider data newer than score;
- contract mismatch;
- search flow;
- account discovery.
...
Create a pure decision function returning a reasoned decision such as:

```ts
type RefreshDecision =
  | { action: "NONE"; publicState: "FRESH"; reason: "WITHIN_SCORE_TTL" }
  | { action: "ENQUEUE"; publicState: "QUEUED"; reason: "NO_PUBLISHED_SCORE" }
  | { action: "ENQUEUE"; publicState: "STALE"; reason: "SCORE_TTL_EXPIRED" }
  | { action: "RECALCULATE"; publicState: "STALE"; reason: "MODEL_CONTRACT_CHANGED" }
  | { action: "REUSE_ACTIVE_JOB"; publicState: "REFRESHING"; reason: string }
  | { action: "BACKOFF"; publicState: "STALE"; reason: "RECENT_FAILURE" };
```

Names may differ, but semantics must be explicit and centralized.

## Required behaviour

- Published score freshness is based on the published snapshot calculation/publication time, not provider TTL.
- Default score TTL: 7 days, centrally configured.
- Reading fresh profile/search/account data causes zero enqueue side effects.
- Stale read returns last score and enqueues at most one refresh.
- Existing queued/active logical job is reused.
- Completed jobs are not re-armed by ordinary reads.
- A recent failed job produces backoff and keeps last score.
- Contract/model incompatibility prefers `RECALCULATE_ONLY` when persisted evidence is compatible; full provider refresh only when actually needed.
- Provider-newer-than-score is a warning/diagnostic, not an unconditional refresh loop.
- Account character polling does not change a completed score back to loading.
- A profile with an old score and active refresh displays stale score plus refreshing indicator.
- Force refresh is moved behind an admin permission contract; coordinate API shape with Agent 04 without implementing IAM broadly.

## Status semantics

Define and document one public state machine.
Avoid ambiguous use of `STALE`.
...
- fresh;
- stale but usable;
- refresh failed with fallback;
- unavailable/insufficient;
- U due to score eligibility.

Provider states must remain separate.

## Tests

Include regression tests for:

1. ten fresh profile reads create zero jobs;
2. stale profile creates exactly one job;
3. concurrent stale reads reuse one logical job;
4. reading after job completion does not enqueue again before 7 days;
5. account navigation does not requeue fresh characters;
6. failed refresh applies backoff;
7. stale score remains visible during refresh;
8. model mismatch chooses recalculate versus full refresh correctly;
9. search and profile use the same policy;
10. cache invalidation does not bypass policy.

## Constraints

- Preserve last-known-good publication.
- Preserve negative cache behaviour.
- No live WCL calls in tests.
- Avoid destructive schema changes.
- If a new timestamp/table is truly required, justify it and provide backward-compatible migration.
- Do not change public scoring mathematics.
