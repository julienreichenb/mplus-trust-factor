# Standalone Cursor prompt — Agent 08

This prompt is self-contained. You are operating inside the dedicated Git worktree
for branch `agent/08-admin-model-lifecycle`.

You are Agent 08: Admin Model Lifecycle for the M+ Trust Factor repository.
...
Agents 04, 09 and 10 must be merged.

## Mission

Finish the website-based score model lifecycle.
Normal activation must require no database command and no VPS `.env` edit.

## Required lifecycle

```text
active immutable model
 -> clone to draft
 -> edit
 -> server validation
 -> real cohort backtest
 -> explicit admin confirmation
 -> transactional activation
 -> previous active archived
 -> audit event
 -> enqueue RECALCULATE_ONLY bulk operation for all persisted characters
```

## Source of truth

- database active model status/version is authoritative;
- remove normal runtime dependence on `ACTIVE_SCORE_MODEL_KEY`;
- environment configuration may bootstrap an empty database only;
- bootstrap must be explicit, idempotent and documented;
- do not mutate active model configuration in place.

## Admin UI

Improve existing `AdminModelsPage.vue`:

- clear active/draft/archived status;
- immutable version history;
- clone/edit draft;
- server-side validation errors;
- real cohort selection/backtest using Agent 10;
- display grade distribution, outliers, confidence/coverage and dimension shifts;
- compare draft versus active;
- activation confirmation summarizing impact;
- show created bulk recalculation job and link to progress;
- prevent double activation;
- permission protected.

## Backend

- transactional one-active-model invariant per model key;
- optimistic concurrency or equivalent protection;
- validated config/schema version;
- activation audit;
- enqueue Agent 09's recalculate-only operation after transaction;
- clear failure semantics if enqueue fails after activation;
- no provider calls during activation request itself.

## Tests

- draft cannot be public/active accidentally;
- invalid draft cannot activate;
- two concurrent activations leave one active;
- previous active archived;
- audit created;
- bulk operation created exactly once;
- no env edit required;
- normal user denied;
- backtest is real, not fixture distribution;
- restart preserves active model;
- empty DB bootstrap documented/tested.

## Constraints

- do not propose new weights or activate v7;
- do not run mass recalculation on live test without explicit user action;
- preserve existing public snapshots until recalculated replacements publish.
