# Standalone Cursor prompt — Agent 09

This prompt is self-contained. You are operating inside the dedicated Git worktree
for branch `agent/09-bulk-character-processing`.

You are Agent 09: Bulk Character Processing for the M+ Trust Factor repository.
...
Agents 03 and 04 must be merged.

## Mission

Implement one resumable, admin-controlled cohort orchestrator for mass refresh and mass recalculation.
...
```ts
type BulkMode = "FULL_REFRESH" | "RECALCULATE_ONLY";

interface BulkCharacterProcessingInput {
  mode: BulkMode;
...
Refine naming and schema as needed.

Semantics:
- numeric threshold: select characters at/above it;
- null threshold: select all persisted characters;
- `FULL_REFRESH`: provider pipeline plus scoring;
- `RECALCULATE_ONLY`: reuse compatible persisted evidence and avoid provider calls;
- incompatible/missing evidence is reported, not silently converted into full refresh unless explicitly configured.

## Required architecture

- dedicated persisted bulk-operation entity and item/checkpoint state;
- deterministic selection snapshot or cursor strategy;
- idempotent enqueue and processing;
- resumable after worker restart;
- batched fan-out with bounded concurrency;
- per-character status and error;
- aggregate progress;
- pause/cancel semantics if practical;
- dry-run count/cost estimate;
- rate/budget enforcement;
- no duplicate active operation with same logical key;
- admin-only API;
- admin UI to create, observe and cancel/pause where supported;
- audit events.

## Integration targets

Use the existing queue/repository patterns and own queue topology changes:
...
- admin API/UI status views
- observability events and metrics

## Model activation use case

Agent 08 will activate a model then enqueue:
- `mode = RECALCULATE_ONLY`
- `minMythicPlusScore = null`
- selected new `scoreModelId`.

Design a stable service/API for that integration.

## Tests

- threshold selection;
- null selects all;
- dry run no child jobs;
- resumability/checkpoint;
- idempotency;
- cancellation/pause;
- no WCL/provider calls in compatible recalculate-only;
- budget stop in full-refresh;
- per-item failure does not corrupt operation;
- progress counts exact;
- admin authorization;
- migration on empty and existing DB.

## Constraints

- do not activate a score model;
- do not change scoring math;
- do not perform a large live run.
