# Standalone Cursor prompt — Agent 01

This prompt is self-contained. You are operating inside the dedicated Git worktree
for branch `agent/01-foundation-repository-inventory`.

You are Agent 01: Foundation Repository Inventory for the M+ Trust Factor repository.
...
Produce an evidence-based inventory and cleanup plan.
This is primarily an audit/documentation task. Do not perform broad deletion or refactoring.

## Inventory scope

Inspect:

- `README.md`, `AGENTS.md`, `.cursor/**`
- `doc/**`, `docs/**`, `agents/**`
- all WCL probe and calibration scripts in `tools/scripts/**`
- `packages/providers/warcraftlogs/src/probe/**`
- shared evidence and production imports
- old generated fixtures/raw artifacts references
- duplicate utilities and repeated adapter generations
- open branches/worktrees visible locally
- open PR #1 and PR #2 and their file lists
- stale wave/integration branch references
- dead npm scripts and unreferenced files where evidence is strong

## Classification

Classify every important candidate as exactly one:
...
For probe files, show:
- imported by production?
- imported by tests?
- invoked by package scripts?
- unique scientific value?
- replacement/superseding file?
- deletion risk?

## Front-end PR decision

Do not merge or close PRs.
Document a concrete consolidation plan:

- PR #1 is the structural base.
- PR #2 contributes useful detail.
- outdated product naming, six-dimension references and stale architecture must be corrected.
- propose the replacement branch and exact retained files/assets.

## Deliverables

Create under a temporary clearly named audit directory, preferably:
`docs/audits/repository-inventory-2026-07/`
...
- `frontend-pr-consolidation.md`
- `proposed-canonical-tree.md`
- `safe-delete-list.txt`
- `requires-human-review.txt`
- `worktree-and-branch-inventory.md`

Do not delete candidates in this agent.

## Validation

- support claims with imports, script references or git evidence;
- distinguish "not found" from "unused";
- run repository tests only if your documentation additions affect validation;
