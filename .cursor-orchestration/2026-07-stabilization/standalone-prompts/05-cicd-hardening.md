# Standalone Cursor prompt — Agent 05

This prompt is self-contained. You are operating inside the dedicated Git worktree
for branch `agent/05-cicd-hardening`.

You are Agent 05: CI/CD Hardening for the M+ Trust Factor repository.
...
Make every commit merged to `main` automatically build and deploy the test environment reliably.
Prepare, but do not activate, the future `prod` branch policy.

Agent 00's CI repair must already be merged.
...
Prepare:
- `prod` branch as production source;
- production workflow only from `prod` or protected manual dispatch tied to a `prod` SHA;
- documented required reviewers/GitHub Environment protection;
- no direct feature-branch production deploy;
- no production deployment during this task.

## Fix current issues

- replace `integration/wave4.3` push trigger with `main` for test;
- remove temporary wave branch assumptions;
- missing required deploy secrets must fail the deploy job, not produce a green no-op;
- verify `/health/ready`, not only `/health/live`;
- verify deployed revision/image SHA;
- ensure migrations run once and fail safely;
- preserve separate test/prod secrets and databases;
- ensure rollback uses immutable previous SHA;
- avoid `git checkout SHA || git pull` ambiguity;
- validate environment file and required variables before touching the running stack;
- add concurrency policy that does not cancel an in-progress production deployment;
- make deploy logs actionable and redact secrets.

## Model lifecycle concern

Do not solve active score-model selection with VPS `.env` edits.
Remove CD responsibilities that mutate the active model.
Normal model activation will be database/admin-driven in Agent 08.

Environment variables may only bootstrap a truly empty database if documented and idempotent.

## Manual-process inventory

Document and automate or explicitly retain:
- secret generation;
- VPS directory preparation;
...
- first-admin bootstrap hook from Agent 04 if its contract is merged.

## Testing

- workflow syntax;
- shellcheck-equivalent validation where available;
- local script dry-run/validation;
- Docker image builds;
- invalid/missing env fails before deployment;
- test deployment smoke path;
...
- no live provider mass calls;
- no application-scoring changes;
- preserve test/prod isolation.
