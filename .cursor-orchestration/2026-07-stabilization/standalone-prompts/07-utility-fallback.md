# Standalone Cursor prompt — Agent 07

This prompt is self-contained. You are operating inside the dedicated Git worktree
for branch `agent/07-utility-fallback`.

You are Agent 07: Utility Fallback Implementation for the M+ Trust Factor repository.
...
Agents 03 and 06 must be merged.
Use Agent 06's reviewed taxonomy and evidence findings.

## Mission

Make Utility reliably calculable when adequate public evidence exists, while minimizing WCL cost.

## Baseline

- Reuse the common scoring/Survival detailed run evidence first.
- Normal baseline: maximum 8 selected runs, one best canonical run per active dungeon.
- Do not refetch an immutable report revision/event stream already cached.
- Produce an explicit baseline eligibility result.
- A complete sample with zero attributable positive actions publishes neutral Utility with appropriate low/moderate confidence and does not fallback.

## Fallback

Trigger only for reviewed `INSUFFICIENT_EVIDENCE_RETRYABLE`.

Initial hard limits:
- maximum 4 additional runs;
- prefer missing or underrepresented active dungeons;
- one additional run per dungeon before taking a second from the same dungeon;
- prefer public, recent, matched reports with predicted event availability;
- stop immediately when Utility becomes publishable;
- stop on WCL rate defer/stop budget;
- persist request cost and selection reasons;
- do not change the canonical Performance run selection.
...
- selected extra run IDs/reasons;
- newly fetched versus cached evidence;
- pages/calls/cost;
...
Own Utility/shared-evidence and refresh-pipeline Utility sections.
Do not modify worker queue topology (`processors.ts`, new queue names, bulk job contracts); Agent 09 owns that area.

## Tests

- baseline publishable: no fallback calls;
- complete zero contribution: score 50, no fallback;
- retryable insufficient baseline: selects bounded extras;
- stops after first sufficient extra run;
- never exceeds four;
- respects active dungeon pool;
- avoids duplicate cached fetch;
- rate-limited/budget exhausted does not loop;
- private/no-public logs do not fallback;
- actor/match failure diagnostics;
- shared Survival evidence reused;
- deterministic selection;
- request-cost ledger exact;
- public U/low-confidence behaviour remains consistent.
