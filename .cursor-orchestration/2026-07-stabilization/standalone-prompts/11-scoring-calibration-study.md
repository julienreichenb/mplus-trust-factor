# Standalone Cursor prompt — Agent 11

This prompt is self-contained. You are operating inside the dedicated Git worktree
for branch `agent/11-scoring-calibration-study`.

You are Agent 11: Scoring Calibration Study for the M+ Trust Factor repository.
...
## Dependency and required user input

Agent 08 must be merged.

Before implementation/execution, request the user's 24-character cohort:

- 8 DPS, 8 tank, 8 healer;
- per role: 2 excellent, 2 good, 2 average, 2 weak/overrated;
- region, realm, name, spec/class, expected label, meta/non-meta, rationale, boost suspicion.

Do not invent this manual cohort.

Then add 24 stratified public profiles for a total initial cohort of 48.

## Mission

Evaluate whether dimensions, Trust Score and S/A/B/C/D/U ranks reflect expert expectations for the target top-20%-pug audience.

## Process

1. Freeze/version the cohort.
2. Ensure evidence freshness under the 7-day policy.
3. Use budget-bound refresh only when necessary and approved.
4. Run active v6 baseline.
5. Analyze overall and each dimension.
6. Identify:
   - false-high profiles;
   - false-low profiles;
   - non-meta suppression;
   - meta inflation;
   - role anomalies;
   - missing-data artefacts;
   - Utility/Survival saturation;
   - boost-flag quality.
7. Run sensitivity and candidate draft configurations.
8. Propose a v7 draft only if evidence supports it.

## Important rules

- Do not force tier quotas.
- Do not activate any candidate model.
- Do not modify model formulas before baseline results are recorded.
- Separate data-quality defects from calibration defects.
- Preserve public wording: boost is suspicion, not fact.
- Treat 48 as exploratory, not definitive scientific validation.

## Deliverables

- versioned cohort;
- raw exported results;
- analysis notebook/script if used;
- Markdown report;
- anomaly list;
- recommended code/data fixes;
- proposed draft model config and rationale;
- expected distribution versus observed;
- follow-up sample-size plan;
- admin backtest instructions.

## Acceptance criteria

- reproducible from immutable snapshot/model IDs;
- all exclusions explained;
- no hidden manual score editing;
- role and meta/non-meta slices;
- confidence and coverage included;
- candidate model remains DRAFT.
