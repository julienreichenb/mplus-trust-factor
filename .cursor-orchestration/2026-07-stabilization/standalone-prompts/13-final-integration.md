# Standalone Cursor prompt — Agent 13

This prompt is self-contained. You are operating inside the dedicated Git worktree
for branch `agent/13-final-integration`.

You are Agent 13: Final Integration and Release Readiness for the M+ Trust Factor repository.
...
Perform final integration validation for the consolidated test environment.
Fix only integration defects with clear root causes; do not introduce new features.

## Required end-to-end scenarios

1. CI:
   - PR checks green;
   - main push builds immutable images;
   - automatic test deploy succeeds;
   - readiness and revision verification succeed.

2. Battle.net/admin:
   - existing test Battle.net user obtains admin through idempotent bootstrap;
   - local bootstrap instructions work after first login;
...
3. Refresh lifecycle:
   - fresh score read creates zero job;
   - account navigation does not reload completed ranks;
   - stale score remains visible;
   - exactly one background refresh;
   - failed refresh backoff;
   - provider states and score state not conflated.

4. Utility:
   - baseline reuses shared evidence;
   - fallback only for retryable insufficiency;
   - maximum four extra runs;
   - cost ledger exact;
   - complete zero contribution does not fallback.

5. Bulk processing:
   - threshold full-refresh dry run;
   - null-threshold recalculate-only dry run;
   - resume/cancel;
   - no providers in compatible recalculate-only.

6. Model lifecycle:
   - clone/edit/validate/backtest;
   - activate a disposable test draft only if user explicitly authorizes;
   - progressive recalculate operation;
   - no VPS env edit.

7. Documentation:
   - canonical docs match code;
   - old PR #1/#2 replacement guidance complete;
   - agent/worktree cleanup documented.
...
- exact production prerequisites, without deploying production;
- commands to remove merged worktrees and branches;
- recommendation: READY_FOR_CONTINUED_TESTING or NOT_READY, with reasons.

## Constraints

- no production deployment;
- no unapproved active model change;
- no mass live WCL operation;
