# Standalone Cursor prompt — Agent 00

This prompt is self-contained. You are operating inside the dedicated Git worktree
for branch `agent/00-foundation-ci-repair`.

You are Agent 00: Foundation CI Repair for the M+ Trust Factor repository.
...
Restore a deterministic green CI gate with the smallest justified change set.
Do not redesign CD, scoring, refresh, IAM or application architecture in this task.
...
1. GitHub Actions PostgreSQL is exposed on `localhost:5432`, but `vitest.config.ts`
   currently forces `DATABASE_URL` to `localhost:5433`.
2. `apps/worker/src/shutdown.test.ts` expects four workers while
   `apps/worker/src/processors.ts` now creates five.

## Required implementation

- Make test configuration respect an explicitly supplied `DATABASE_URL`.
- Preserve a sensible local fallback without overriding CI.
- Replace the brittle worker-count assertion with an assertion over the expected
  queue/worker identities or another maintainable contract.
- Search for other hard-coded test ports or stale fixed worker counts.
- Run the full quality sequence used by CI.
- Ensure the IAM seed and health readiness test target the actual CI database.
- Do not disable, skip or weaken tests to obtain green status.
- Do not make dependency audit blocking in this task unless it is the actual failure.
...
- `apps/api/src/app.ts`
- `apps/worker/src/shutdown.test.ts`
...
- `pnpm build`
- `pnpm typecheck`
- `pnpm db:migrate` against an empty PostgreSQL database
- `pnpm test`
- `pnpm abilities:validate`
- no hard-coded CI/local environment collision
- no skipped test introduced
- a concise root-cause note is added to the appropriate CI/operations documentation
- final commit is ready for review but not merged

## Stop conditions

Stop if the fix requires a destructive migration or broad application refactor.
