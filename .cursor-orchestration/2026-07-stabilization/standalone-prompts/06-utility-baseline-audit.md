# Standalone Cursor prompt — Agent 06

This prompt is self-contained. You are operating inside the dedicated Git worktree
for branch `agent/06-utility-baseline-audit`.

You are Agent 06: Utility Baseline Audit for the M+ Trust Factor repository.
...
Determine why Utility is unavailable/weak for real characters and specify the minimal evidence fallback.
This is an audit and instrumentation task, not the final fallback implementation.

## Questions to answer

For the baseline shared run sample:

- How many selected runs have reusable WCL detailed evidence?
- Which required Utility event streams are missing?
- Is evidence absent because:
  - no public report;
  - report matching failed;
  - actor resolution failed;
  - event pagination/truncation;
  - wrong event types queried;
  - unsupported spell catalog;
  - truly zero observed contribution;
  - rate/budget stop;
  - cache incompatibility?
- Can Survival evidence bundles satisfy Utility without additional calls?
- For failures, how many extra runs would have made Utility publishable?
- What is the call/event-page cost distribution?
- Which fallback run selection yields the best evidence per WCL request?

## Required explicit baseline states

Recommend and prototype diagnostics for:
...
- `packages/providers/warcraftlogs/src/evidence/**`
- `packages/providers/warcraftlogs/src/probe/utility-v3_2-observed-*`
- Utility catalogs and opportunity/audit modules
- Survival shared-evidence ingestion
- WCL request-cost accounting and rate-budget code
- external request cache/persistence
- existing probe scripts and fixtures

## Deliverables

- an audit document with failure taxonomy;
- machine-readable diagnostic schema proposal;
- request-cost table;
- baseline publication criteria;
- fallback selection algorithm;
- recommended cap validation, starting from max 4 extra runs;
- exact implementation file plan for Agent 07;
- regression fixture recommendations.

You may add non-invasive diagnostics and tests if needed.
Do not change public score output or call additional live WCL at scale.

## Live validation

If credentials and explicit user-approved fixture characters are available, perform only a small bounded validation.
Record every call and stop before configured warning thresholds.
Otherwise produce commands for later execution and state that live validation was not performed.
