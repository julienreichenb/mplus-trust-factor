# Standalone Cursor prompt — Agent 10

This prompt is self-contained. You are operating inside the dedicated Git worktree
for branch `agent/10-calibration-harness`.

You are Agent 10: Calibration Harness for the M+ Trust Factor repository.
...
Agent 07 must be merged.

## Mission

Create a reproducible calibration/backtest harness that evaluates real persisted or fixture cohorts without changing the active model.

## Inputs

Support a versioned cohort manifest containing:

- region, realm, character;
- role/class/spec;
- expected qualitative label;
- meta/non-meta marker;
- rationale;
- suspected boost marker;
- source: user-selected or stratified-auto;
...
- per-character overall score, grade and confidence;
- all dimension scores/confidences;
- coverage and refresh states;
- boost flags;
- active model/version;
- expected versus actual label;
- rank confusion/ordering summaries;
- grade distribution;
- role/class/spec/meta slices;
- missing-data slices;
- Utility baseline/fallback cost;
- CSV, JSON and Markdown report.

## Backtest modes

- persisted-snapshot-only;
- explicit refresh-then-evaluate, disabled by default and budget-bound;
- alternate draft model evaluation without activation.

## Statistical/pragmatic checks

At minimum:
- monotonic ordering against expert labels;
- outlier list;
- dimension saturation/floor rates;
- confidence versus coverage;
- meta versus non-meta;
- role slices;
- grade distribution without forced quotas;
- sensitivity/ablation for model weights;
- bootstrap confidence intervals where sample size permits, clearly labelled exploratory.

## Integration

Expose a clean service/CLI API that Agent 08 can call from the admin backtest endpoint.
Do not edit the admin page/service beyond an adapter interface agreed in docs.

## Tests

- deterministic replay;
- no provider calls in persisted mode;
- draft model does not become active;
- malformed cohort validation;
- reproducible output;
- anonymized/public-safe report;
- U and low-confidence cases retained;
- output schema versioned.

## Constraints

- no conclusion about final calibration yet;
