...
<template>
  <section class="page" aria-labelledby="v2-explain-title">
    <header class="page__header">
      <h1 id="v2-explain-title">Scoring V2 diagnostics</h1>
      <p>
        Evidence quality, selection matrix, datasets, fact-set versions, dimension confidence, and
        V1/V2 comparison. Report codes are admin-only.
      </p>
    </header>
    ...
    <section v-if="list" class="panel" aria-labelledby="manifest-list-title">
      <h2 id="manifest-list-title">Manifests</h2>
    ...
    <template v-if="diagnostics">
      <section class="panel" aria-labelledby="matrix-title">
        <h2 id="matrix-title">2×dungeon matrix</h2>
        <p>
          Coverage {{ diagnostics.coverageState }} · selected
          {{ diagnostics.selectedSlotCount }}/{{ diagnostics.expectedSlotCount }} · hash
          {{ diagnostics.manifestContentHash.slice(0, 12) }}…
        </p>
        <div v-for="[dungeon, cells] in matrixByDungeon" :key="dungeon" class="matrix-row">
          ...
          <ul>
            <li v-for="cell in cells" :key="`${cell.dungeonSlug}-${cell.slotIndex}`">
              Slot {{ cell.slotIndex }} · {{ cell.state }} · +{{ cell.keyLevel ?? "—" }}
      ...
      <section class="panel" aria-labelledby="rejected-title">
        <h2 id="rejected-title">Rejected candidates</h2>
      ...
      <section class="panel" aria-labelledby="datasets-title">
        <h2 id="datasets-title">Datasets (state / cost / pages / truncation)</h2>
      ...
      <section class="panel" aria-labelledby="factsets-title">
        <h2 id="factsets-title">Fact-set versions</h2>
      ...
      <section class="panel" aria-labelledby="dims-title">
        <h2 id="dims-title">Dimension calculation and confidence</h2>
      ...
      <section class="panel" aria-labelledby="compare-title">
        <h2 id="compare-title">V1 / V2 comparison</h2>
        <pre>{{ JSON.stringify(diagnostics.comparison, null, 2) }}</pre>
      </section>

      <section class="panel" aria-labelledby="queue-title">
        <h2 id="queue-title">Batch / queue state</h2>
        <pre v-if="diagnostics.batchQueue">{{ JSON.stringify(diagnostics.batchQueue, null, 2) }}</pre>
        <p v-else class="empty">No analysis batch linked.</p>
      </section>

      <section class="panel" aria-labelledby="calib-title">
        <h2 id="calib-title">Calibration links</h2>
        <ul>
