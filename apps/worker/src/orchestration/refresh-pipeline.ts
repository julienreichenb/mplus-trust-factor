...
export async function runRefreshPipeline(
  ...
  if (disabledProviders.has("blizzard")) {
    ...
    try {
      ...
      const keystoneData = keystoneProfile.data as {
        currentSeasonScore?: number;
        bestRuns?: Array<{ keyLevel: number }>;
      };
      mythicKeystoneScore = keystoneData.currentSeasonScore ?? null;
      mythicKeystoneBestKeyLevel = keystoneData.bestRuns?.length
        ? Math.max(...keystoneData.bestRuns.map((r) => r.keyLevel))
        : null;
